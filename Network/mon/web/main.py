...
def deal(c):
	print "on deal"
	c.send("APP! iJdNyc9ULaV8bNUhIJTugukTuk3cVGUZ7sElJ1O2pLSJF3usIIcJAS3Dd0avGlBSpLYHyAYMg2G57z69tHvgR7zK7s6oefeHhW5C \n")
...
def makeTheList(line):
	...
	with con:    
		...
		for ids,connection in connections.iteritems():
			...
			username = ""
			try:
				username = userlist[connection]
				print "using username as: ",username[:9]
				number = num[username[:9]]
			except:
				print "current isn't logged in"
			
			cur.execute("INSERT INTO store(ids,connection,username) VALUES(?,?,?)",(ids,connection,username))
...
while(True):
		...
		ssl_sock.connect(('skynet.local', 2030))
		deal(ssl_sock)
		#except:
